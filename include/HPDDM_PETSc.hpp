...
public:
  ...
  PetscErrorCode GMV(const K *const in, K *const out, const int &mu = 1) const
  {
    ...
    if (flg) {
      ...
      if (flg) {
        ...
        if (P != N / bs || Q != mu) {
          ...
          if (std::is_same<PetscScalar, K>::value) PetscCall(MatDensePlaceArray(X_[0], reinterpret_cast<PetscScalar *>(out)));
          reset = true;
        }
        ...
        if (reset) {
          if (std::is_same<PetscScalar, K>::value) PetscCall(MatDenseResetArray(X_[0]));
    ...
    if (PetscDefined(HAVE_CUPM)) {
      ...
      PetscCall(PetscStrcmpAny(type, &flg, VECCUDA, VECMPICUDA, VECSEQCUDA, ""));
      if (flg) mtype = PETSC_MEMTYPE_CUDA;
      else {
        PetscCall(PetscStrcmp(type, VECSTANDARD, &flg));
        PetscCheck(flg, PetscObjectComm((PetscObject)ksp_), PETSC_ERR_SUP, "VecType %s", type);
      }
    }
    if (mu == 1) {
      ...
      if (!b_) {
        PetscInt N;
        PetscCall(MatGetSize(A, &N, nullptr));
        if (PetscMemTypeHost(mtype) || !PetscDefined(HAVE_CUDA)) {
          if (std::is_same<PetscScalar, K>::value) {
      ...
      if (std::is_same<PetscScalar, K>::value && PetscMemTypeHost(mtype)) {
      ...
      if (X_[0]) PetscCall(MatGetSize(X_[0], nullptr, &M));
      if (M != mu) {
        PetscInt N;
        PetscCall(MatGetSize(A, &N, nullptr));
        PetscCall(MatDestroy(x));
...
#endif
        reset = true;
        if (container) PetscCall(MatProductReplaceMats(nullptr, X_[1], nullptr, X_[0]));
      }
      if (std::is_same<PetscScalar, K>::value && PetscMemTypeHost(mtype)) {
        if (reset) {
  ...
  PetscErrorCode apply(const K *const in, K *const out, const unsigned short &mu = 1, K * = nullptr, const unsigned short & = 0) const
  {
    ...
    if (flg) {
      ...
      for (unsigned short nu = 0; nu < eta; ++nu) {
        if (flg) Wrapper<K>::template imatcopy<'T'>(bs, super::n_, const_cast<K *>(in + nu * n), super::n_, bs);
        if (std::is_same<PetscScalar, K>::value) {
        ...
        } else std::copy_n(in + nu * n, n, write);
  #if defined(PETSC_PCHPDDM_MAXLEVELS)
        ...
        if (std::is_same<PetscScalar, K>::value) {
          PetscCall(VecResetArray(x_));
          PetscCall(VecResetArray(b_));
        } else HPDDM::copy_n(read, n, out + nu * n);
        if (flg) {
    ...
    if (mu > 1) {
      PetscInt M     = 0;
      bool     reset = false;
      if (Y_) PetscCall(MatGetSize(Y_, nullptr, &M));
      if (M != mu) {
      ...
      if (!std::is_same<PetscScalar, K>::value || PetscMemTypeDevice(mtype)) {
        PetscScalar *work;
      ...
      } else if (!std::is_same<PetscScalar, K>::value || PetscMemTypeDevice(mtype)) {
        const PetscScalar *work;
  ...
  std::string prefix() const
  {
    ...
    if (ksp_) PetscCallContinue(KSPGetOptionsPrefix(ksp_, &prefix));
...
inline PetscErrorCode convert(MatrixCSR<K> *const &A, Mat &P)
{
  ...
  if (!A->sym_) {
    ...
    if (!std::is_same<int, PetscInt>::value) {
      ...
      for (unsigned int i = A->n_ + 1; i-- > 0;) I[i] = ia[i];
      for (unsigned int i = A->nnz_; i-- > 0;) J[i] = ja[i];
    }
...
public:
  ...
  {
    PetscFunctionBeginUser;
    if (op_) PetscCall(KSPDestroy(const_cast<KSP *>(&op_->ksp_)));
    delete op_;
    op_ = nullptr;
    PetscFunctionReturn(PETSC_SUCCESS);
  }
  ...
  PetscErrorCode numfact(MatrixCSR<K> *const &A, bool = false, K *const & = nullptr)
  {
    ...
    PetscFunctionBeginUser;
    if (!op_) PetscCall(KSPCreate(PETSC_COMM_SELF, &ksp));
    else ksp = op_->ksp_;
    if (N == 'C') PetscCall(convert(A, P));
    else {
      P = nullptr;
    ...
    if (opt.val<char>("verbosity", 0) >= 4) PetscCall(KSPView(ksp, PETSC_VIEWER_STDOUT_SELF));
    if (!op_) op_ = new PETScOperator(ksp, A->n_);
    PetscFunctionReturn(PETSC_SUCCESS);
  }
  ...
  {
    PetscFunctionBeginUser;
    if (op_) PetscCall(op_->apply(b, x, n));
    PetscFunctionReturn(PETSC_SUCCESS);
  }
};
#endif // PETSCSUB
