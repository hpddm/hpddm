...
public:
  ...
  void numfact(unsigned int nz, int *I, int *J, K *C)
  {
    ...
    if (S == 'S') id_->sym = opt.val<char>("operator_spd", 0) ? 1 : 2;
    else id_->sym = 0;
    MUMPS_STRUC_C<K>::mumps_c(id_);
    ...
    id_->job = 4;
    if (opt.val<char>("verbosity", 0) < 3) id_->icntl[2] = 0;
    MUMPS_STRUC_C<K>::mumps_c(id_);
    if (DMatrix::rank_ == 0 && id_->infog[0] != 0) std::cerr << "BUG MUMPS, INFOG(1) = " << id_->infog[0] << std::endl;
    id_->icntl[2] = 0;
    delete[] I;
  }
    /* Function: solve
  ...
  void solve(K *const rhs, const unsigned short &n)
  {
    ...
    if (D == DMatrix::DISTRIBUTED_SOL) {
      ...
      } else DMatrix::redistribute<0>(sol_loc, rhs);
      for (unsigned short nu = 1; nu < n; ++nu) DMatrix::redistribute<0>(sol_loc + nu * info, rhs + nu * *DMatrix::ldistribution_);
      delete[] sol_loc;
      delete[] isol_loc;
    } else {
      id_->icntl[20] = 0;
  ...
  {
    if (n > id_->nrhs) {
      if (id_->nrhs) delete[] reinterpret_cast<K *>(id_->rhs);
...
public:
  ...
  void numfact(MatrixCSR<K> *const &A, bool detection = false, K *const &schur = nullptr)
  {
    ...
    id_->cntl[2]   = -1.0e-6;
    if (N == 'C') std::for_each(A->ja_, A->ja_ + A->nnz_, [](int &i) { ++i; });
    id_->jcn     = A->ja_;
    ...
    if (id_->job == -1) {
      ...
      I_        = new int[A->nnz_];
      id_->nz   = A->nnz_;
      for (int i = 0; i < A->n_; ++i) std::fill(I_ + A->ia_[i] - (N == 'F'), I_ + A->ia_[i + 1] - (N == 'F'), i + 1);
      id_->irn = I_;
      if (schur) {
    ...
    MUMPS_STRUC_C<K>::mumps_c(id_);
    delete[] listvar;
    if (id_->infog[0] != 0) std::cerr << "BUG MUMPS, INFOG(1) = " << id_->infog[0] << std::endl;
    id_->icntl[2] = 0;
    if (N == 'C') std::for_each(A->ja_, A->ja_ + A->nnz_, [](int &i) { --i; });
  }
  template <char N = HPDDM_NUMBERING>
  int inertia(MatrixCSR<K> *const &A)
  {
    ...
    v              = 1;
    numfact<N>(A, true);
    if (remove) opt.remove("mumps_icntl_13");
    return id_->infog[11];
  }
