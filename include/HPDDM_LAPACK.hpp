...
#define HPDDM_GENERATE_EXTERN_LAPACK(C, T, U, SYM, ORT) \
  ...
  void HPDDM_F77(C##sytrf)(const char *, const int *, T *, const int *, int *, T *, const int *, int *) HPDDM_NOEXCEPT; \
  void HPDDM_F77(C##sytrs)(const char *, const int *, const int *, const T *, const int *, const int *, T *, const int *, int *) HPDDM_NOEXCEPT; \
  void HPDDM_F77(C##sytri)(const char *, const int *, T *, const int *, const int *, T *, int *) HPDDM_NOEXCEPT; \
  void HPDDM_F77(C##potrf)(const char *, const int *, T *, const int *, int *) HPDDM_NOEXCEPT; \
  ...
  void HPDDM_F77(C##ppsv)(const char *, const int *, const int *, T *, T *, const int *, int *) HPDDM_NOEXCEPT; \
...
public:
  QR(int n, const K *const cpy = nullptr) : n_(n), lwork_(workspace()), a_(new K[n_ * (n_ + 1) + lwork_]), tau_(a_ + n_ * n_), work_(tau_ + n_)
  {
    ...
    if (cpy)
      for (unsigned int i = 0; i < n_; ++i) {
        ...
        for (unsigned int j = i + 1; j < n_; ++j) a_[j * n_ + i] = a_[i * n_ + j] = cpy[i * n_ + j];
      }
  }
  QR(const QR &) = delete;
  ~QR() { delete[] a_; }
  ...
  void decompose()
  {
    ...
    for (unsigned int i = 1; i < n_; ++i) {
      if (std::abs(a_[(n_ + 1) * i]) < max * 1.0e-6) jpvt.emplace_back(i);
      else if (std::abs(a_[(n_ + 1) * i]) > max / 1.0e-6) {
        jpvt.clear();
        max = std::abs(a_[(n_ + 1) * i]);
        i   = 0;
      } else max = std::max(std::abs(a_[(i + 1) * n_]), max);
    }
...
public:
  ...
  void numfact(MatrixCSR<K> *const &A, bool detection = false, K *const & = nullptr)
  {
    ...
    a_ = new K[n_ * n_]();
    if (A->nnz_ == n_ * n_) {
      if (N == 'C') Wrapper<K>::template omatcopy<'T'>(n_, n_, A->a_, n_, a_, n_);
      else std::copy_n(A->a_, A->nnz_, a_);
    } else {
      for (unsigned int i = 0; i < A->n_; ++i) {
        for (unsigned int j = A->ia_[i] - (N == 'F'); j < A->ia_[i + 1] - (N == 'F'); ++j) {
          if (!transpose) a_[i + (A->ja_[j] - (N == 'F')) * n_] = A->a_[j];
          else a_[i * n_ + (A->ja_[j] - (N == 'F'))] = A->a_[j];
        }
      }
    }
    int info;
    if (!A->sym_) {
      ...
      } else Lapack<K>::potrf("L", &n_, a_, &n_, &info);
    }
  }
  template <char = HPDDM_NUMBERING>
  int inertia(MatrixCSR<K> *const &)
  {
    return 0;
  }
  ...
  {
    int nrhs = n, info;
    if (type_ == 1) Lapack<K>::sytrs("L", &n_, &nrhs, a_, &n_, ipiv_, x, &n_, &info);
    else if (type_ == 2) Lapack<K>::potrs("L", &n_, &nrhs, a_, &n_, x, &n_, &info);
    else Lapack<K>::getrs("N", &n_, &nrhs, a_, &n_, ipiv_, x, &n_, &info);
  }
...
public:
  ...
  {
    MatrixCSR<K> *E;
    if (I == nullptr && J == nullptr) E = new MatrixCSR<K>(n, n, n * n, C, nullptr, nullptr, S == 'S');
    else E = new MatrixCSR<K>(n, n, I[n] - (numbering_ == 'F'), C, I, J, S == 'S');
  ...
  #define HPDDM_GENERATE_LAPACK(C, T, B, U, SYM, ORT) \
    ...
    template <> \
    inline U Lapack<T>::lange(const char *norm, const int *m, const int *n, const T *a, const int *lda, U *work) \
    { \
      return HPDDM_F77(C##lange)(norm, m, n, a, lda, work); \
    } \
    template <> \
    inline U Lapack<T>::lan(const char *norm, const char *uplo, const int *m, const T *a, const int *lda, U *work) \
    { \
      return HPDDM_F77(C##lan##SYM)(norm, uplo, m, a, lda, work); \
    } \
    template <> \
    inline void Lapack<T>::gst(const int *itype, const char *uplo, const int *n, T *a, const int *lda, T *b, const int *ldb, int *info) \
    { \
      HPDDM_F77(C##SYM##gst)(itype, uplo, n, a, lda, b, ldb, info); \
    } \
    template <> \
    inline void Lapack<T>::trd(const char *uplo, const int *n, T *a, const int *lda, U *d, U *e, T *tau, T *work, const int *lwork, int *info) \
    { \
    ...
    template <> \
    inline void Lapack<T>::gehrd(const int *n, const int *ilo, const int *ihi, T *a, const int *lda, T *tau, T *work, const int *lwork, int *info) \
    { \
    ...
    template <> \
    inline void Lapack<T>::getri(const int *n, T *a, const int *lda, int *ipiv, T *work, const int *lwork, int *info) \
    { \
    ...
    template <> \
    inline void Lapack<T>::sytri(const char *uplo, const int *n, T *a, const int *lda, int *ipiv, T *work, int *info) \
    { \
    ...
    template <> \
    inline void Lapack<T>::pstrf(const char *uplo, const int *n, T *a, const int *lda, int *piv, int *rank, const U *tol, U *work, int *info) \
    { \
    ...
    template <> \
    inline void Lapack<T>::posv(const char *uplo, const int *n, const int *nrhs, T *a, const int *lda, T *b, const int *ldb, int *info) \
    { \
    ...
    template <> \
    inline void Lapack<T>::pptrs(const char *uplo, const int *n, const int *nrhs, T *ap, T *b, const int *ldb, int *info) \
    { \
      HPDDM_F77(C##pptrs)(uplo, n, nrhs, ap, b, ldb, info); \
    } \
    template <> \
    inline void Lapack<T>::ppsv(const char *uplo, const int *n, const int *nrhs, T *ap, T *b, const int *ldb, int *info) \
    { \
    ...
    template <> \
    inline void Lapack<T>::geqrt(const int *m, const int *n, const int *nb, T *a, const int *lda, T *t, const int *ldt, T *work, int *info) \
    { \
  ...
  #define HPDDM_GENERATE_LAPACK_COMPLEX(C, T, B, U) \
    ...
    template <> \
    inline void Lapack<U>::pocon(const char *uplo, const int *n, const U *a, const int *lda, U *anorm, U *rcond, U *work, int *iwork, int *info) \
    { \
      HPDDM_F77(B##pocon)(uplo, n, a, lda, anorm, rcond, work, iwork, info); \
    } \
    template <> \
    inline void Lapack<T>::pocon(const char *uplo, const int *n, const T *a, const int *lda, U *anorm, U *rcond, T *work, U *rwork, int *info) \
    { \
