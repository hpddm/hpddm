...
struct EmptyOperator : OptionsPrefix<K> {
  ...
  static constexpr bool start(const U *const, U *const, const unsigned short & = 1)
  {
    return false;
  }
...
private:
                               ...
                               const underlying_type<K> *const norm, const K *const res, short *const conv, const short sentinel)
  {
    ...
    if (verbosity > 2) {
      ...
      for (unsigned short nu = 0; nu < mu; ++nu) {
        if (conv[nu] != -sentinel) ++tmp[0];
        else if (std::abs(res[nu]) > beta) {
          beta   = std::abs(res[nu]);
          tmp[1] = nu;
        }
      }
      if (tol > 0.0) std::cout << method << ": " << std::setw(3) << j << " " << beta << " " << norm[tmp[1]] << " " << beta / norm[tmp[1]] << " < " << tol;
      else std::cout << method << ": " << std::setw(3) << j << " " << beta << " < " << -tol;
      if (mu > 1) {
        std::cout << " (rhs #" << tmp[1] + 1;
        if (tmp[0]) std::cout << ", " << tmp[0] << " converged rhs";
        std::cout << ")";
      }
      std::cout << std::endl;
    }
                                              ...
                                              const underlying_type<K> *const norm, const K *const res, const int ldh, K *const work, const unsigned short t)
  {
    ...
    if (T == 3 || T == 6) {
      for (unsigned short nu = 0; nu < mu / t; ++nu) {
        ...
        if (((tol > underlying_type<K>() && pt[nu] / norm[nu] <= tol) || (tol < underlying_type<K>() && pt[nu] <= -tol))) ++conv;
      }
    } else if (t <= 1) {
      conv = mu - d;
      for (unsigned short nu = 0; nu < d; ++nu) {
        int dim = nu + 1;
        pt[nu]  = Blas<K>::nrm2(&dim, res + nu * ldh, &i_1);
        if (((tol > underlying_type<K>() && pt[nu] / norm[nu] <= tol) || (tol < underlying_type<K>() && pt[nu] <= -tol))) ++conv;
      }
    } else {
      std::fill_n(work, d, K());
      for (unsigned short nu = 0; nu < t; ++nu) {
        int dim = nu + 1;
        Blas<K>::axpy(&dim, &(Wrapper<K>::d_1), res + nu * ldh, &i_1, work, &i_1);
      }
      *pt = Blas<K>::nrm2(&d, work, &i_1);
      if (((tol > underlying_type<K>() && *pt / *norm <= tol) || (tol < underlying_type<K>() && *pt <= -tol))) ++conv;
    }
#if !HPDDM_PETSC || defined(PETSCSUB) || defined(MU_SLEPC)
    if (verbosity > 2) {
      ...
      if (tol > 0.0) {
        ...
        for (unsigned short k = 1; k < d / t; ++k) {
          if (pt[j] / norm[j] < pt[k] / norm[k]) j = k;
        }
        max = pt + j;
      ...
      if (d != t || t != mu) {
        ...
        if (conv > d) std::cout << ", " << t * conv - d << " converged rhs";
        if (d != mu) std::cout << ", " << mu - d << " deflated rhs";
        std::cout << ")";
      }
      std::cout << std::endl;
    }
  ...
  static void computeMin(const K *const *const h, K *const s, const short *const hasConverged, const int &mu, const int &deflated = -1, const int &shift = 0)
  {
    ...
    if (deflated != -1) {
      ...
      for (unsigned short nu = 0; nu < mu; ++nu) {
        ...
        if (dim) Blas<K>::trsv("U", "N", "N", &(dim -= shift), *h + shift * (1 + ldh) + (ldh / mu) * nu, &ldh, s + nu, &mu);
      }
  }
  template <bool excluded, class Operator, class K, class T>
      ...
      if (deflated == -1) {
        ...
        if (variant == HPDDM_VARIANT_LEFT) {
          for (unsigned short nu = 0; nu < mu; ++nu)
            if (hasConverged[nu]) {
              int dim = std::abs(hasConverged[nu]);
              Blas<K>::gemv("N", &n, &dim, &(Wrapper<K>::d_1), *v + nu * n, &ldv, s + nu, &mu, &(Wrapper<K>::d_1), x + nu * n, &i_1);
            }
        } else {
          for (unsigned short nu = 0; nu < mu; ++nu) {
            int dim = std::abs(hasConverged[nu]);
            Blas<K>::gemv("N", &n, &dim, &(Wrapper<K>::d_1), *v + nu * n, &ldv, s + nu, &mu, &(Wrapper<K>::d_0), work + nu * n, &i_1);
          }
          if (variant == HPDDM_VARIANT_RIGHT) HPDDM_CALL(A.template apply<excluded>(work, correction, mu));
          for (unsigned short nu = 0; nu < mu; ++nu)
            if (hasConverged[nu]) Blas<K>::axpy(&n, &(Wrapper<K>::d_1), correction + nu * n, &i_1, x + nu * n, &i_1);
        }
      } else {
        int dim = *hasConverged;
        if (deflated == mu) {
          if (variant == HPDDM_VARIANT_LEFT) Blas<K>::gemm("N", "N", &n, &mu, &dim, &(Wrapper<K>::d_1), *v, &n, s, &ldh, &(Wrapper<K>::d_1), x, &n);
          else {
            Blas<K>::gemm("N", "N", &n, &mu, &dim, &(Wrapper<K>::d_1), *v, &n, s, &ldh, &(Wrapper<K>::d_0), work, &n);
            if (variant == HPDDM_VARIANT_RIGHT) HPDDM_CALL(A.template apply<excluded>(work, correction, mu));
            Blas<K>::axpy(&(dim = mu * n), &(Wrapper<K>::d_1), correction, &i_1, x, &i_1);
          }
        } else {
                                ...
                                K *const work, const MPI_Comm &comm, const int &deflated = -1)
  {
    ...
    if (C && U) {
      ...
      if (deflated == -1) {
        if (same) std::fill_n(s, shift * mu, K());
        else {
          if (!excluded && n) {
            K *const pt = A.getScaling() ? work : v[shift];
            if (A.getScaling()) Wrapper<K>::diag(n, reinterpret_cast<const underlying_type<K> *>(A.getScaling()), v[shift], pt, mu);
            for (unsigned short nu = 0; nu < mu; ++nu) {
              if (std::abs(hasConverged[nu])) {
                K alpha = norm[nu];
                Blas<K>::gemv(&(Wrapper<K>::transc), &n, &shift, &alpha, C + nu * n, &ldv, pt + nu * n, &i_1, &(Wrapper<K>::d_0), s + nu, &mu);
              }
            }
          } else std::fill_n(s, shift * mu, K());
          ignore(MPI_Allreduce(MPI_IN_PLACE, s, shift * mu, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm));
        }
        if (!excluded && n)
          for (unsigned short nu = 0; nu < mu; ++nu) {
            if (std::abs(hasConverged[nu])) {
              int diff = std::abs(hasConverged[nu]) - shift;
              Blas<K>::gemv("N", &shift, &diff, &(Wrapper<K>::d_2), h[shift] + nu * dim, &ldh, s + shift * mu + nu, &mu, &(Wrapper<K>::d_1), s + nu, &mu);
            }
          }
      } else {
        ...
        if (!same) {
          if (!excluded && n) {
            ...
            Blas<K>::trmm("R", "U", "N", "N", &n, &deflated, &(Wrapper<K>::d_1), reinterpret_cast<K *>(norm), &ldh, work, &n);
            Wrapper<K>::diag(n, reinterpret_cast<const underlying_type<K> *>(A.getScaling()), work, mu);
            Blas<K>::gemm(&(Wrapper<K>::transc), "N", &bK, &deflated, &n, &(Wrapper<K>::d_1), C, &n, work, &n, &(Wrapper<K>::d_0), s, &ldh);
            for (unsigned short i = 0; i < deflated; ++i) std::copy_n(s + i * ldh, bK, work + i * bK);
          } else std::fill_n(work, bK * deflated, K());
          ignore(MPI_Allreduce(MPI_IN_PLACE, work, bK * deflated, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm));
          for (unsigned short i = 0; i < deflated; ++i) std::copy_n(work + i * bK, bK, s + i * ldh);
          beta = Wrapper<K>::d_1;
        }
  ...
  {
    if (out) Wrapper<T>::diag(n, d, in, out);
                            ...
                            underlying_type<K> *const norm, const unsigned short mu, const unsigned short k, bool &allocate)
  {
    ...
    if (variant == HPDDM_VARIANT_LEFT) {
      ...
      if (d)
        for (unsigned short nu = 0; nu < mu; ++nu) {
          norm[nu / k] = 0.0;
          for (int i = 0; i < n; ++i) norm[nu / k] += d[i] * HPDDM::norm(v[i + nu * n]);
        }
      else
        for (unsigned short nu = 0; nu < mu; ++nu) norm[nu / k] = HPDDM::real(Blas<K>::dot(&n, v + nu * n, &i_1, v + nu * n, &i_1));
    } else {
      if (k <= 1) work = const_cast<K *>(b);
      else {
        std::fill_n(work, n, K());
        for (unsigned short nu = 0; nu < k; ++nu) Blas<K>::axpy(&n, &(Wrapper<K>::d_1), b + nu * n, &i_1, work, &i_1);
      }
                                ...
                                const underlying_type<K> *const d, K *const work, const MPI_Comm &comm)
  {
    if (excluded || !n) {
      std::fill_n(H, k * mu, K());
      if (id == 1)
        for (unsigned short i = 0; i < k; ++i) ignore(MPI_Allreduce(MPI_IN_PLACE, H + i * mu, mu, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm));
      else ignore(MPI_Allreduce(MPI_IN_PLACE, H, k * mu, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm));
    } else {
      if (id == 1) {
        for (unsigned short i = 0; i < k; ++i) {
          if (d)
            for (unsigned short nu = 0; nu < mu; ++nu) {
              H[i * mu + nu] = K();
              for (int j = 0; j < n; ++j) H[i * mu + nu] += d[j] * Wrapper<K>::conj(B[(i * mu + nu) * n + j]) * v[nu * n + j];
            }
          else
            for (unsigned short nu = 0; nu < mu; ++nu) H[i * mu + nu] = Blas<K>::dot(&n, B + (i * mu + nu) * n, &i_1, v + nu * n, &i_1);
          ignore(MPI_Allreduce(MPI_IN_PLACE, H + i * mu, mu, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm));
          for (unsigned short nu = 0; nu < mu; ++nu) {
            K alpha = -H[i * mu + nu];
            Blas<K>::axpy(&n, &alpha, B + (i * mu + nu) * n, &i_1, v + nu * n, &i_1);
          }
        }
      } else {
        int      ldb = mu * n;
                                     ...
                                     const underlying_type<K> *const d, K *const work, const MPI_Comm &comm)
  {
    if (excluded || !n) {
      ...
      if (id == 1)
        for (unsigned short i = 0; i < k; ++i) {
          ignore(MPI_Allreduce(MPI_IN_PLACE, work, mu * mu, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm));
          Wrapper<K>::template omatcopy<'N'>(mu, mu, work, mu, H + mu * i, ldh);
        }
      else {
        ignore(MPI_Allreduce(MPI_IN_PLACE, work, k * mu * mu, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm));
      ...
      if (id == 1) {
        for (unsigned short i = 0; i < k; ++i) {
          if (d) Wrapper<K>::diag(n, d, v, pt, mu);
          Blas<K>::gemm(&(Wrapper<K>::transc), "N", &mu, &mu, &n, &(Wrapper<K>::d_1), B + i * mu * n, &n, pt, &n, &(Wrapper<K>::d_0), work, &mu);
          ignore(MPI_Allreduce(MPI_IN_PLACE, work, mu * mu, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm));
          Blas<K>::gemm("N", "N", &n, &mu, &mu, &(Wrapper<K>::d_2), B + i * mu * n, &n, work, &mu, &(Wrapper<K>::d_1), v, &n);
          Wrapper<K>::template omatcopy<'N'>(mu, mu, work, mu, H + mu * i, ldh);
        }
      } else {
        if (d) Wrapper<K>::diag(n, d, v, pt, mu);
        const int tmp = k * mu;
        Blas<K>::gemm(&(Wrapper<K>::transc), "N", &tmp, &mu, &n, &(Wrapper<K>::d_1), B, &n, pt, &n, &(Wrapper<K>::d_0), work, &tmp);
        ignore(MPI_Allreduce(MPI_IN_PLACE, work, mu * tmp, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm));
        Blas<K>::gemm("N", "N", &n, &mu, &tmp, &(Wrapper<K>::d_2), B, &n, work, &tmp, &(Wrapper<K>::d_1), v, &n);
                   ...
                   const underlying_type<K> *const d, K *const work, const MPI_Comm &comm)
  {
    if (tol < underlying_type<K>(-0.9)) {
      ...
      int info;
      Lapack<K>::pstrf("U", &k, R, &k, piv, &rank, &(Wrapper<underlying_type<K>>::d_0), reinterpret_cast<underlying_type<K> *>(work), &info);
      while (rank > 1 && HPDDM::abs(R[(rank - 1) * (k + 1)] / R[0]) <= tol) --rank;
      Lapack<K>::lapmt(&i_1, &n, &k, Q, &n, piv);
      if (!excluded && n) Blas<K>::trsm("R", "U", "N", "N", &n, &rank, &(Wrapper<K>::d_1), R, &k, Q, &n);
      return true;
    }
  }
  template <char T, class K>
                ...
                const MPI_Comm &comm, bool update = true, const int mu = 1)
  {
    ...
    if (id == HPDDM_QR_CHOLQR) {
      ...
      if (!excluded && n && update)
        for (unsigned short nu = 0; nu < mu; ++nu) Blas<K>::trsm("R", "U", "N", "N", &n, &rank, &(Wrapper<K>::d_1), R + nu * k * ldr, &ldr, Q + nu * n, &ldv);
    } else {
      if (!work) work = R;
      K *const pt = (d || work == R) ? work + k * k * mu : work;
      for (unsigned short xi = 0; xi < rank; ++xi) {
        if (xi > 0) orthogonalization<excluded>(id - 1, n, xi, mu, Q, Q + xi * ldv, work + xi * k * mu, d, pt, comm);
        if (d)
          for (unsigned short nu = 0; nu < mu; ++nu) {
            work[xi * (k + 1) * mu + nu] = K();
            for (int j = 0; j < n; ++j) work[xi * (k + 1) * mu + nu] += d[j] * HPDDM::norm(Q[xi * ldv + nu * n + j]);
          }
        else
          for (unsigned short nu = 0; nu < mu; ++nu) work[xi * (k + 1) * mu + nu] = Blas<K>::dot(&n, Q + xi * ldv + nu * n, &i_1, Q + xi * ldv + nu * n, &i_1);
        ignore(MPI_Allreduce(MPI_IN_PLACE, work + xi * (k + 1) * mu, mu, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm));
        for (unsigned short nu = 0; nu < mu; ++nu) {
          work[xi * (k + 1) * mu + nu] = HPDDM::sqrt(work[xi * (k + 1) * mu + nu]);
          if (HPDDM::real(work[xi * (k + 1) * mu + nu]) < underlying_type<K>(HPDDM_EPS)) rank = xi;
        }
        if (rank != xi)
          for (unsigned short nu = 0; nu < mu; ++nu) {
            K alpha = K(1.0) / work[xi * (k + 1) * mu + nu];
            Blas<K>::scal(&n, &alpha, Q + xi * ldv + nu * n, &i_1);
          }
      }
      if (work != R) {
        for (unsigned short nu = 0; nu < mu; ++nu)
          for (unsigned short i = 0; i < rank; ++i)
            for (unsigned short j = 0; j < rank; ++j) R[(nu * k + i) * ldr + j] = work[(i * k + j) * mu + nu];
      }
    }
    return rank;
  }
                      ...
                      const unsigned short shift = 0)
  {
    ...
    if (excluded) std::fill_n(sn + i * mu, mu, 0.0);
      ...
      for (unsigned short nu = 0; nu < mu; ++nu) {
        sn[i * mu + nu] = 0.0;
        for (int j = 0; j < n; ++j) sn[i * mu + nu] += d[j] * HPDDM::norm(v[i + 1][nu * n + j]);
      }
    else
      for (unsigned short nu = 0; nu < mu; ++nu) sn[i * mu + nu] = HPDDM::real(Blas<K>::dot(&n, v[i + 1] + nu * n, &i_1, v[i + 1] + nu * n, &i_1));
                           ...
                           const unsigned short shift = 0)
  {
    ...
    if (info != mu) return true;
    for (unsigned short nu = 0; nu < mu; ++nu) std::fill(H[i] + (i + 1) * mu + nu * ldh + nu + 1, H[i] + (nu + 1) * ldh, K());
    if (save)
                          ...
                          int rank, int div, const MPI_Comm &comm)
  {
    ...
    for (unsigned int *pt = find_zero(global, k); pt != global + k; pt = find_zero(global, k), ++j) {
      if ((rank < k * div && (rank % div) == j) || (rank >= k * div && (j + (k - 1) * div == rank))) {
        while (pt != global + k) {
          ...
          for (unsigned short nu = 0; nu < k; ++nu, ++swap) {
            if (*swap > 1 || (*swap == 1 && global[nu] > 1)) break;
          }
          if (swap != local + k) {
      ...
      ignore(MPI_Allreduce(local, global, k, MPI_UNSIGNED, MPI_SUM, comm));
    }
  }
  ...
  static void preprocess(const Operator &A, const K *const b, K *&sb, K *const x, K *&sx, const int &mu, unsigned short &k, const MPI_Comm &comm)
  {
    ...
    if (k < 2 || size == 1 || mu > 1) {
      ...
      {
        unsigned int max = 0;
        for (unsigned short nu = 0; nu < k && max < k; ++nu) max += global[nu];
        if (max < k) {
          k      = std::max(1U, max);
          global = local + k;
  ...
  static void postprocess(const Operator &A, const K *const b, K *&sb, K *const x, K *&sx, unsigned short &k)
  {
    if (sb != b) {
      const int n = A.getDof();
      std::fill_n(x, n, K());
      for (unsigned short j = 0; j < k; ++j) Blas<K>::axpy(&n, &(Wrapper<K>::d_1), sx + j * n, &i_1, x, &i_1);
      delete[] sb;
      delete[] sx;
    }
  }
  ...
  {
    if (!mu) return;
    HPDDM::underlying_type<K> *storage = new HPDDM::underlying_type<K>[2 * static_cast<std::size_t>(mu)];
    ...
    if (!hpddm_method_id<Operator>::value) {
      ...
      } else MPI_Allreduce(MPI_IN_PLACE, storage, 2 * mu, Wrapper<K>::mpi_underlying_type(), MPI_MAX, comm);
    }
    int rank;
    MPI_Comm_rank(comm, &rank);
    if (rank == 0) {
      std::string header = "Final residual";
      if (mu > 1) header = header + "s";
      const std::string prefix = A.prefix();
      if (prefix.size() > 0) header = header + " (" + prefix + ")";
      header = header + ": ";
                              ...
                              const unsigned short norm)
  {
    const int n   = A.getDof();
    int       dim = mu * n;
    K        *tmp = new K[static_cast<std::size_t>(mu) * n];
    A.GMV(x, tmp, mu);
    Blas<K>::axpy(&dim, &(Wrapper<K>::d_2), b, &i_1, tmp, &i_1);
    std::fill_n(storage, 2 * mu, underlying_type<K>());
    if (norm == HPDDM_COMPUTE_RESIDUAL_L1) {
      for (int i = 0; i < n; ++i) {
        for (unsigned short nu = 0; nu < mu; ++nu) {
          storage[2 * nu + 1] += std::abs(tmp[nu * n + i]);
          if (std::abs(b[nu * n + i]) > HPDDM_EPS * HPDDM_PEN) storage[2 * nu] += std::abs(b[nu * n + i] / underlying_type<K>(HPDDM_PEN));
          else storage[2 * nu] += std::abs(b[nu * n + i]);
        }
      }
      ...
      for (int i = 0; i < n; ++i) {
        for (unsigned short nu = 0; nu < mu; ++nu) {
          ...
          if (std::abs(b[nu * n + i]) > HPDDM_EPS * HPDDM_PEN) storage[2 * nu] += std::norm(b[nu * n + i] / underlying_type<K>(HPDDM_PEN));
          else storage[2 * nu] += std::norm(b[nu * n + i]);
        }
      }
    }
    delete[] tmp;
...
public:
  ...
  K *sx = nullptr;
  K *sb = nullptr;
  if (k) preprocess<excluded>(A, b, sb, x, sx, mu, k, comm);
  else {
    sx = x;
    sb = const_cast<K *>(b);
    k  = 1;
  ...
  case HPDDM_KRYLOV_METHOD_NONE: {
    const bool allocate = A.template start<excluded>(sb, sx, k * mu);
  ...
  case HPDDM_KRYLOV_METHOD_RICHARDSON:
    it = Richardson<excluded>(A, sb, sx, k * mu, comm);
    break;
  #endif
  case HPDDM_KRYLOV_METHOD_BFBCG:
    it = BFBCG<excluded>(A, sb, sx, k * mu, comm);
    break;
  case HPDDM_KRYLOV_METHOD_BGCRODR:
    it = BGCRODR<excluded>(A, sb, sx, k * mu, comm);
    break;
  case HPDDM_KRYLOV_METHOD_GCRODR:
    it = GCRODR<excluded>(A, sb, sx, k * mu, comm);
    break;
  case HPDDM_KRYLOV_METHOD_BCG:
    it = BCG<excluded>(A, sb, sx, k * mu, comm);
    break;
  case HPDDM_KRYLOV_METHOD_CG:
    it = CG<excluded>(A, sb, sx, k * mu, comm);
    break;
  case HPDDM_KRYLOV_METHOD_BGMRES:
    it = BGMRES<excluded>(A, sb, sx, k * mu, comm);
    break;
  default:
    it = GMRES<excluded>(A, sb, sx, k * mu, comm);
  }
  HPDDM_CALL(it);
  if (HPDDM_IT(it, A) >= 0) {
    ...
    if (!excluded && k != 10) printResidual(A, b, x, mu, k, comm);
  #endif
  }
  std::cout.flags(ff);
  #if defined(PETSC_PCHPDDM_MAXLEVELS)
...
static int solve(const Operator &A, const K *const b, K *const x, const int &, const MPI_Comm &comm)
{
  ...
  if (!excluded && k == HPDDM_COMPUTE_RESIDUAL_L2) printResidual(A, b, x, 1, HPDDM_COMPUTE_RESIDUAL_L2, comm);
  #endif
  std::cout.flags(ff);
  return it;
}
