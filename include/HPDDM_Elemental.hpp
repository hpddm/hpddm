...
public:
  Elemental() : grid_(), A_(), B_(), d_(), P_(), Q_(), loc2glob_(), bs_(), type_() { El::Initialize(); }
  ~Elemental()
  {
    if (loc2glob_) {
      delete[] loc2glob_;
  ...
  void numfact(unsigned short bs, int *I, int *loc2glob, int *J, K *&C)
  {
    ...
    loc2glob_ = loc2glob;
    bs_       = bs;
    if (S == 'S') type_ = Option::get()->val<char>("operator_spd", 0) ? 1 : 2;
    if (type_ == 1) {
  ...
  else if (type_ == 2) El::ldl::SolveAfter(*A_, *d_, *P_, *B_, false);
  else El::lu::SolveAfter(El::NORMAL, *A_, *P_, *Q_, *B_);
  B_->ReservePulls((loc2glob_[1] - loc2glob_[0] + 1) * n * bs_);
  for (int j = 0; j < n; ++j) {
    for (int i = 0; i < (loc2glob_[1] - loc2glob_[0] + 1) * bs_; ++i) B_->QueuePull(loc2glob_[0] * bs_ + i, j);
  }
  #if !HPDDM_INEXACT_COARSE_OPERATOR
