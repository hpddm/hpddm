...
{
#if HPDDM_DENSE
template <template <class> class, template <class> class, char, class>
class Dense;
#endif
/* Class: Schwarz
 *
...
public:
  ...
  explicit Schwarz(const Subdomain<K> &s) :
    super(s),
    d_()
#if HPDDM_SCHWARZ
    ,
  ...
  void exchange() const
  {
    ...
    for (unsigned short i = 0, size = Subdomain<K>::map_.size(); i < size; ++i) {
      ...
      } else Subdomain<K>::rq_[i] = MPI_REQUEST_NULL;
    }
    for (unsigned short i = 0, size = Subdomain<K>::map_.size(); i < size; ++i) {
      int        index;
      MPI_Status st;
  ...
  {
    unsigned int n = 0;
    for (const auto &i : Subdomain<K>::map_) n += i.second.size();
    std::vector<int> p;
    if (n && !Subdomain<K>::map_.empty()) {
      ...
      *buff                           = new underlying_type<K>[2 * n];
      ...
      for (unsigned short i = 0, size = Subdomain<K>::map_.size(); i < size; ++i) {
        ...
        for (int j = 0; j < Subdomain<K>::map_[index].second.size(); ++j) d[Subdomain<K>::map_[index].second[j]] += buff[index][j];
      }
      p.reserve(Subdomain<K>::dof_);
      for (int i = 0; i < Subdomain<K>::dof_; ++i)
        if ((HPDDM::abs(D[i] - 1.0) > HPDDM_EPS && HPDDM::abs(D[i]) > HPDDM_EPS) || HPDDM::abs(d[i] - 1.0) > HPDDM_EPS) p.emplace_back(i);
      delete[] d;
      int rank;
  ...
  void callNumfact(MatrixCSR<K> *const &A = nullptr)
  {
    ...
    case HPDDM_SCHWARZ_METHOD_SORAS:
      type_ = (A ? Prcndtnr::OS : Prcndtnr::SY);
      break;
    ...
    case HPDDM_SCHWARZ_METHOD_NONE:
      ...
      return;
    default:
      type_ = (A && (m == HPDDM_SCHWARZ_METHOD_ORAS || m == HPDDM_SCHWARZ_METHOD_OSM) ? Prcndtnr::OG : Prcndtnr::GE);
    }
    m = opt.val<unsigned short>("reuse_preconditioner");
    if (m <= 1) super::s_.template numfact<N>(type_ == Prcndtnr::OS || type_ == Prcndtnr::OG ? A : Subdomain<K>::a_);
    if (m >= 1) opt["reuse_preconditioner"] += 1;
  ...
  void multiplicityScaling(underlying_type<K> *const d) const
  {
    ...
    for (unsigned short i = 0, size = Subdomain<K>::map_.size(); i < size; ++i) {
      ...
      for (unsigned int j = 0; j < Subdomain<K>::map_[index].second.size(); ++j) {
        if (std::abs(send[j]) < HPDDM_EPS) d[Subdomain<K>::map_[index].second[j]] = 0.0;
        else d[Subdomain<K>::map_[index].second[j]] /= 1.0 + d[Subdomain<K>::map_[index].second[j]] * recv[j] / send[j];
      }
    }
    MPI_Waitall(Subdomain<K>::map_.size(), Subdomain<K>::rq_ + Subdomain<K>::map_.size(), MPI_STATUSES_IGNORE);
  ...
  {
    if (excluded) super::co_->template IcallSolver<excluded>(super::uc_, mu, rq);
    else {
  ...
  typename super::co_type::return_type buildTwo(const MPI_Comm &comm, MatrixCSR<K> *const &A = nullptr)
  {
    ...
    Option           &opt    = *Option::get();
    const std::string prefix = super::prefix();
    if (opt.val<unsigned short>(prefix + "level_2_aggregate_size") != 1) opt.remove(prefix + "level_2_schwarz_method");
  #endif
    ...
    if (super::co_) {
      ...
      if (super::co_ && p > 1 && opt.val<unsigned short>(prefix + "level_2_aggregate_size", p) == 1 && (method == HPDDM_SCHWARZ_METHOD_RAS || method == HPDDM_SCHWARZ_METHOD_ASM) && opt.val<char>(prefix + "level_2_krylov_method", HPDDM_KRYLOV_METHOD_GMRES) != HPDDM_KRYLOV_METHOD_NONE) {
        ...
        if (opt.set(prefix + "level_2_schwarz_coarse_correction") && A) {
          ...
          if (filename.size() > 0) opt.parse(filenameNeumann);
          ...
          if (overlap.size()) MPI_Isend(overlap.data(), overlap.size(), Wrapper<K>::mpi_type(), 0, 300, coNeumann->getCommunicator(), &rs);
          if (filename.size() > 0) {
  ...
  bool start(const K *const b, K *const x, const unsigned short &mu = 1) const
  {
    ...
    if (!excluded && Subdomain<K>::a_->ia_) {
      ...
      for (const std::pair<const unsigned int, K> &p : map)
        for (unsigned short nu = 0; nu < mu; ++nu) x[nu * Subdomain<K>::dof_ + p.first] = b[nu * Subdomain<K>::dof_ + p.first] / p.second;
    }
    exchange(x, mu);
    if (super::co_) {
      unsigned short    k      = 1;
  ...
  int apply(const K *const in, K *const out, const unsigned short &mu = 1, K *work = nullptr) const
  {
    ...
    if ((!super::co_ && !super::cc_) || correction == static_cast<char>(-1)) {
      if (type_ == Prcndtnr::NO) std::copy_n(in, mu * Subdomain<K>::dof_, out);
        ...
        if (!excluded) {
          ...
          } else super::s_.solve(in, out, mu);
          Subdomain<K>::exchange(out, mu); // out = A \ in
        }
      }
    } else {
      int n = mu * Subdomain<K>::dof_;
      if (!work) work = const_cast<K *>(in);
      else if (!excluded) std::copy_n(in, n, work);
  ...
  void scaleIntoOverlap(const MatrixCSR<K> *const &A, MatrixCSR<K> *&B) const
  {
    ...
    for (const pairNeighbor &neighbor : Subdomain<K>::map_)
      for (unsigned int i : neighbor.second)
        if (d_[i] > HPDDM_EPS) intoOverlap.insert(i);
    ...
    for (k = 0; k < intoOverlap.size(); ++k) {
      ...
      for (unsigned int j = A->ia_[*it] - (N == 'F'); j < A->ia_[*it + 1] - (N == 'F'); ++j) {
        ...
        if (std::abs(value) > HPDDM_EPS && intoOverlap.find(A->ja_[j] - (N == 'F')) != intoOverlap.cend()) tmp[k].emplace_back(A->ja_[j], value);
      }
      iPrev += tmp[k].size();
    }
    int nnz = iPrev;
  ...
  static PetscErrorCode destroy(PC_HPDDM_Level *const ctx, PetscBool all)
  {
    ...
    if (ctx->P && ctx->P->dof_ != -1) {
      if (all) {
        if (!std::is_same<K, PetscReal>::value) delete[] ctx->P->d_;
        ctx->P->dtor();
        delete ctx->P;
        ctx->P = nullptr;
      } else {
  ...
  PetscErrorCode structure(const IS interior, IS is, const Mat D, Mat N, PC_HPDDM_Level **const levels)
  {
    ...
    if (!Subdomain<K>::rq_) {
      ...
      Subdomain<K>::dof_ = m;
      if (!std::is_same<PetscScalar, PetscReal>::value) d = new PetscReal[Subdomain<K>::dof_];
      else PetscCall(VecGetArray(levels[0]->D, reinterpret_cast<PetscScalar **>(&d)));
      d_ = d;
      ...
      if (!ismatis) {
        ...
        for (PetscInt i = 0; i < Subdomain<K>::dof_ / bs; ++i) idx[i] = ptr[i * bs] / bs;
      ...
      for (PetscInt i = 0; i < Subdomain<K>::dof_; i += bs) v.emplace_back(std::make_pair(ptr[i], i));
      ...
      if (ismatis) {
        ...
        if (!sym) {
          for (const std::pair<const PetscInt, std::unordered_set<PetscInt>> &p : boundary) {
            ...
            for (const PetscInt &i : p.second) map[i].insert(cols, cols + ncols);
          ...
          for (PetscInt i = 0; i < Subdomain<K>::dof_; i += bs) {
            ...
            if (it != boundary.cend()) {
              for (const PetscInt &i : it->second) map[i].insert(cols, cols + ncols);
            ...
            for (PetscInt j = 0; j < ncols; j += bs) {
              ...
              if (it != boundary.cend()) {
                for (const PetscInt &k : it->second)
                  for (PetscInt j = 0; j < bs; ++j) map[k].insert(i + j);
              }
            }
        ...
        for (unsigned short i = 0; i < Subdomain<K>::map_.size(); ++i) {
          ...
          for (PetscInt j = 0; j < numprocs[1 + sorted[i]]; ++j) {
            for (PetscInt k = 0; k < bs; ++k) Subdomain<K>::map_[i].second.emplace_back(indices[1 + sorted[i]][j] * bs + k);
          }
        }
        delete[] sorted;
        PetscCall(ISLocalToGlobalMappingRestoreBlockInfo(l2g, &nproc, &procs, &numprocs, &indices));
        ...
        PetscCall(ISGetIndices(is, &ptr));
        for (PetscInt i = 0; i < Subdomain<K>::dof_; i += bs) {
          if (ptr[i] >= rstart && ptr[i] < rend) std::fill_n(d + i, bs, 1.0);
          else std::fill_n(d + i, bs, 0.0);
        }
      }
      PetscCall(ISRestoreIndices(is, &ptr));
...
public:
  typename super::co_type::return_type buildTwo(const MPI_Comm &comm, Mat D, PetscInt n, PetscInt M, PC_HPDDM_Level **const levels)
  {
    ...
    if (!flg) {
      ...
      if (!flg) PetscCall(super::template buildTwo<false, MatrixMultiplication<Schwarz<K>, K>>(this, comm, D, n, M, levels));
      else PetscCall(super::template buildTwo<false, BddProjection<Schwarz<K>, K>>(this, comm, D, n, M, levels));
  #if defined(PETSC_HAVE_HTOOL)
    } else {
      struct ClassWithPtr {
  ...
  PetscErrorCode solveGEVP(IS is, Mat N, std::vector<Vec> initial, PC_HPDDM_Level **const levels, Mat weighted, Mat rhs)
  {
    ...
    if (!levels[0]->parent->deflation) {
        ...
        if (solve == 1) {
          ...
          if (h->A[1]) {
            ...
            if (!flg) {
              ...
              for (PetscInt i = 0; i < n; ++i) PetscCall(MatSetValuesBlocked(A, n, rows, 1, rows + i, vals + m * n * i, INSERT_VALUES));
            ...
            if (levels[0]->threshold >= PetscReal()) {
              ...
              if (levels[0]->nu == nconv) PetscCall(PetscInfo(svd, "HPDDM: Not enough converged singular triplets to satisfy the threshold criterion\n"));
            } else levels[0]->nu = std::min(nconv, nev);
      ...
      if (!eps) PetscCall(EPSCreate(PETSC_COMM_SELF, &eps));
      PetscCall(EPSSetOptionsPrefix(eps, prefix));
      PetscCall(PetscObjectTypeCompare((PetscObject)N, MATIS, &ismatis));
      if (!ismatis) {
        Mat compact;
        ...
        if (weighted && !h) PetscCall(EPSSetOperators(eps, N, weighted));
      } else {
        PetscCall(MatISGetLocalToGlobalMapping(N, &l2g, nullptr));
        ...
        if (!rhs) PetscCall(MatDestroy(&weighted));
      ...
      if (weighted || ismatis) {
        if (levels == levels[0]->parent->levels && levels[0]->parent->share) {
          ...
          if (h) PetscCall(KSPSetReusePreconditioner(ksp[0], PETSC_TRUE));
        }
          ...
          if (flg) {
            ...
            if (PetscDefined(HAVE_MUMPS)) {
              ...
              if (!type) {
                ...
                PetscCall(PCSetFromOptions(pc));
              } else PetscCall(PetscStrcmp(type, MATSOLVERMUMPS, &flg));
            }
  #if PetscDefined(HAVE_MUMPS)
            if (flg) {
      ...
      if (ctx) {
        ctx->status = 'b';
        while (!ctx->work->empty() || ctx->status != 'c') PetscCall(MatMult_Sum(weighted, nullptr, nullptr));
      }
      if (levels[0]->threshold >= PetscReal()) {
        ...
        if (levels[0]->nu == nconv && solve == 1) PetscCall(PetscInfo(eps, "HPDDM: Not enough converged eigenpairs to satisfy the threshold criterion\n"));
      } else levels[0]->nu = std::min(nconv, nev);
      if (solve == 1) PetscCall(PetscInfo(eps, "HPDDM: Using %" PetscInt_FMT " out of %" PetscInt_FMT " converged eigenvectors\n", levels[0]->nu, nconv));
    } else {
      PetscInt n;
      solve = 1;
    ...
    if (!levels[0]->parent->deflation) {
      ...
      for (PetscInt i = 0, flg = PETSC_FALSE; i < levels[0]->nu; ++i) {
        ...
        if (!flg) {
          ...
          if (HPDDM::abs(eigi) > std::numeric_limits<PetscReal>::epsilon()) flg = PETSC_TRUE;
  ...
  static PetscErrorCode next(Mat *A, Mat *N, PetscInt i, PetscInt *const n, PC_HPDDM_Level **const levels)
  {
    ...
    if (levels[i - 1]->P) {
      ...
      if (ierr != PETSC_ERR_ARG_WRONG && levels[i]->ksp) {
        ...
        if (rstart == rend) fail[1] = 1;
      }
    } else ierr = PetscErrorCode(PETSC_SUCCESS);
    ...
    if (fail[0]) { /* building level i + 1 failed because there was no deflation vector */
      *n = i;
      if (i > 1 && N) PetscCall(MatDestroy(N));
    } else PetscCall(ierr);
    if (i > 1 && A) PetscCall(MatDestroy(A));
  #if defined(PETSC_USE_LOG)
    if (!levels[0]->parent->log_separate) PetscCall(PetscLogEventEnd(PC_HPDDM_PtAP, levels[i]->ksp, nullptr, nullptr, nullptr));
  #endif
    ...
    if (i + 1 < *n && levels[i - 1]->P) {
      ...
      if (!levels[0]->parent->aux || algebraic) {
  ...
  #if defined(PETSC_USE_LOG)
        if (!levels[0]->parent->log_separate) PetscCall(PetscLogEventBegin(PC_HPDDM_PtBP, levels[i]->ksp, nullptr, nullptr, nullptr));
  #endif
        ...
        delete coNeumann;
        if (i + 2 == *n) PetscCall(MatDestroy(N));
        if (*A) levels[i]->P = levels[i - 1]->P->co_->getSubdomain()->P;
        else levels[i]->P = nullptr;
        PetscCallMPI(MPI_Wait(&rs, MPI_STATUS_IGNORE));
  #if defined(PETSC_USE_LOG)
        if (!levels[0]->parent->log_separate) PetscCall(PetscLogEventEnd(PC_HPDDM_Next, levels[i]->ksp, nullptr, nullptr, nullptr));
  #endif
      }
    }
    PetscFunctionReturn(PETSC_SUCCESS);
...
static PetscErrorCode MatMult_Sum(Mat A, Vec x, Vec y)
{
  ...
  if (!p->work->empty() || p->status == 'a') {
    PetscInt m = 0;
    if (p->A[1]) PetscCall(MatGetLocalSize(p->A[1], nullptr, &m));
  ...
  if (p->status == 'a') {
    ...
    for (unsigned short i = 0, size = map.size(); i < size; ++i) {
      ...
      for (unsigned int j = 0; j < map[index].second.size(); ++j) out[map[index].second[j]] += buffer[index][j];
    }
    HPDDM::Wrapper<PetscScalar>::diag(n, d, out);
    PetscCall(VecRestoreArrayWrite(y, &out));
  } else if (!p->work->empty()) PetscCall(MatDenseRestoreArrayRead(p->A[2], &in));
  PetscCallMPI(MPI_Waitall(map.size(), rq + map.size(), MPI_STATUSES_IGNORE));
  PetscFunctionReturn(PETSC_SUCCESS);
}
