 ...
 *    K              - Scalar type. */
HPDDM_CLASS_COARSE_OPERATOR(Solver, S, K) class CoarseOperator : public coarse_operator_type<HPDDM_TYPES_COARSE_OPERATOR(Solver, S, downscaled_type<K>)> {
public:
#if HPDDM_PETSC
  ...
  void constructionCommunicatorCollective(const unsigned short *const pt, unsigned short size, MPI_Comm &in, MPI_Comm *const out = nullptr)
  {
    ...
    if (sizeComm != size && in != MPI_COMM_NULL) {
      ...
      for (unsigned short i = 1, j = 1, k = 0; j < sizeComm; ++i) {
        if (pt[i] != 0) array[j++] = i - k;
        else if (count && super::ldistribution_[k + 1] == i) ++k;
      }
      MPI_Group_incl(oldComm, sizeComm, array, &newComm);
      MPI_Group_free(&oldComm);
      if (out) MPI_Comm_create(in, newComm, out);
      else {
        MPI_Comm tmp;
        ...
        if (tmp != MPI_COMM_NULL) {
          MPI_Comm_dup(tmp, &in);
          MPI_Comm_free(&tmp);
        } else in = MPI_COMM_NULL;
      }
      MPI_Group_free(&newComm);
      delete[] array;
    } else if (out) MPI_Comm_dup(in, out);
  }
  /* Function: transfer
         *
...
public:
  ...
  ~CoarseOperator()
  {
    ...
    if (isFinalized) std::cerr << "Function " << __func__ << " in " << __FILE__ << ":" << __LINE__ << " should be called before MPI_Finalize()" << std::endl;
    else {
      if (gatherComm_ != scatterComm_ && gatherComm_ != MPI_COMM_NULL) MPI_Comm_free(&gatherComm_);
      if (scatterComm_ != MPI_COMM_NULL) MPI_Comm_free(&scatterComm_);
