...
inline int IterativeMethod::CG(const Operator &A, const K *const b, K *const x, const int &mu, const MPI_Comm &comm)
{
  ...
  if (A.ksp_->guess_zero) {
    A.ksp_->rnorm = static_cast<PetscReal>(*dir / *norm);
    for (unsigned short nu = 1; nu < mu; ++nu) A.ksp_->rnorm = std::max(A.ksp_->rnorm, PetscReal(dir[nu] / norm[nu]));
    PetscCall(KSPLogResidualHistory(A.ksp_, A.ksp_->rnorm));
    PetscCall(KSPMonitor(A.ksp_, 0, A.ksp_->rnorm));
  } else {
    for (unsigned short nu = 0; nu < mu; ++nu)
      if (static_cast<PetscReal>(res[nu] / norm[nu]) < A.ksp_->rtol) dir[nu] = std::numeric_limits<underlying_type<K>>::epsilon() / underlying_type<K>(1000.0);
    A.ksp_->rnorm = static_cast<PetscReal>(*std::max_element(res, res + mu));
    PetscCall(KSPLogResidualHistory(A.ksp_, A.ksp_->rnorm));
    PetscCall(KSPMonitor(A.ksp_, 0, A.ksp_->rnorm));
  }
  ...
  if (std::find_if(dir, dir + mu, [](const underlying_type<K> &v) { return v < static_cast<underlying_type<K>>(std::pow(std::numeric_limits<underlying_type<K>>::epsilon(), 2)); }) == dir + mu) {
    while (i < HPDDM_MAX_IT(it, A)) {
      ...
      std::for_each(dir, dir + mu, [](underlying_type<K> &d) { d = HPDDM::sqrt(d); });
      checkConvergence<2>(id[0], i, i, HPDDM_TOL(tol, A), mu, res, dir, hasConverged, HPDDM_MAX_IT(it, A));
#if HPDDM_PETSC
      PetscCall(KSPLogResidualHistory(A.ksp_, static_cast<PetscReal>(*dir)));
      PetscCall(KSPMonitor(A.ksp_, i, static_cast<PetscReal>(*dir)));
      PetscCall((*A.ksp_->converged)(A.ksp_, i, static_cast<PetscReal>(*dir), &A.ksp_->reason, A.ksp_->cnvP));
      if (A.ksp_->reason)
        std::for_each(hasConverged, hasConverged + mu, [&](short &c) {
          if (c == -HPDDM_MAX_IT(it, A)) c = i;
        });
      else if (A.ksp_->converged == KSPConvergedSkip) std::fill_n(hasConverged, mu, -HPDDM_MAX_IT(it, A));
#endif
...
inline int IterativeMethod::BCG(const Operator &A, const K *const b, K *const x, const int &mu, const MPI_Comm &comm)
{
  ...
  for (unsigned short nu = mu; nu > 0; --nu) std::copy_backward(rho + (nu * (nu - 1)) / 2, rho + (nu * (nu + 1)) / 2, rho + nu * mu - (mu - nu));
  for (unsigned short i = 0; i < mu; ++i)
    for (unsigned short j = 0; j < i; ++j) rho[i + j * mu] = Wrapper<K>::conj(rho[j + i * mu]);
  ...
  while (HPDDM_IT(i, A) <= HPDDM_MAX_IT(m[1], A)) {
    ...
    if (converged) break;
      ...
      for (unsigned short nu = mu; nu > 0; --nu) std::copy_backward(rhs + (nu * (nu - 1)) / 2, rhs + (nu * (nu + 1)) / 2, rhs + nu * mu - (mu - nu));
      for (unsigned short i = 0; i < mu; ++i)
        for (unsigned short j = 0; j < i; ++j) rhs[i + j * mu] = Wrapper<K>::conj(rhs[j + i * mu]);
...
inline int IterativeMethod::BFBCG(const Operator &A, const K *const b, K *const x, const int &mu, const MPI_Comm &comm)
{
  ...
  PetscCall(KSPLogResidualHistory(A.ksp_, A.ksp_->rnorm));
  PetscCall(KSPMonitor(A.ksp_, 0, A.ksp_->rnorm));
  if (A.ksp_->rnorm > PetscReal(10.0) * std::numeric_limits<PetscReal>::epsilon()) {
    if (tol[0] <= static_cast<typename std::remove_reference<decltype(*tol)>::type>(-0.9) && deflated != mu) A.ksp_->reason = KSP_DIVERGED_BREAKDOWN;
  ...
  while (HPDDM_IT(i, A) <= HPDDM_MAX_IT(m[1], A) && deflated != 0) {
    if (!excluded) HPDDM_CALL(A.GMV(p, q, deflated));
...
inline int IterativeMethod::PCG(const Operator &A, const K *const f, K *const x, const MPI_Comm &comm)
{
  ...
  while (i <= it) {
    if (!excluded) {
      ...
      MPI_Allreduce(MPI_IN_PLACE, alpha, i - 1, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm);
      std::fill_n(alpha, 2, K());
      MPI_Allreduce(MPI_IN_PLACE, alpha, 2, Wrapper<K>::mpi_type(), Wrapper<K>::mpi_op(MPI_SUM), comm);
      A.template project<excluded, 'T'>(storage[0]);
    }
    A.template computeDot<excluded>(&resRel, zCurr, zCurr, comm);
    resRel = HPDDM::sqrt(resRel);
    if (verbosity > 2) std::cout << "PCG: " << std::setw(3) << i << " " << resRel << " " << resInit << " " << resRel / resInit << " < " << tol << std::endl;
    if (resRel / resInit <= tol) break;
    else ++i;
    if (!excluded) {
      A.allocateSingle(pCurr);
      p.emplace_back(pCurr);
  ...
  if (std::is_same<ptr_type, K *>::value) A.template computeSolution<excluded>(f, x);
  else A.template computeSolution<excluded>(storage[1], x);
  delete[] alpha;
  for (auto zCurr : z) clean(zCurr);
  for (auto pCurr : p) clean(pCurr);
  clean(storage[0]);
  A.end(allocate);
  return std::min(i, it);
}
