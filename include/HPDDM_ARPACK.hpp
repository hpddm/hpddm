...
public:
  ...
  void solve(MatrixCSR<K> *const &A, MatrixCSR<K> *const &B, K **&ev, const MPI_Comm &communicator, Solver<K> *const &s = nullptr, std::ios_base::openmode mode = std::ios_base::out)
  {
    ...
    if (4 * Eigensolver<K>::nu_ > Eigensolver<K>::n_) Eigensolver<K>::nu_ = std::max(1, Eigensolver<K>::n_ / 4);
    ...
    if (Wrapper<K>::is_complex) rwork = new underlying_type<K>[Eigensolver<K>::n_];
