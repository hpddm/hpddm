...
protected:
  ...
  void numfact(unsigned int nrow, integer_type *I, int *loc2glob, integer_type *J, K *C, unsigned short *neighbors)
  {
    ...
    if (off_ > 1) {
      unsigned int accumulate = 0;
      {
          ...
          if (range.size() > 1) {
            ...
            for (it = range.cbegin(); it < range.cend() - 1; it += 2) {
              ...
              for (unsigned int i = 0; i < size; ++i) {
                ...
                if (T == 1 && (*it)[i] > di_[1]) ia[accumulate + i] -= di_[2];
              }
          ...
          if (mu_ < off_) {
            ...
            for (unsigned int i = 0; i < nrow; ++i) {
              for (unsigned int j = ia[i]; j < ia[i + 1]; ++j) {
                ...
                if (T == 1 && idx > di_[1]) ja[j - (super::numbering_ == 'F')] -= di_[2];
              }
            }
          } else ja = I + nrow + 1;
          a = C;
  ...
  #ifdef DMKL_PARDISO
        if (factorize && (a != C || Option::get()->set(prefix + "schwarz_method"))) range_[2] = 1;
  #endif
        if (mu_ < off_) {
          loc2glob[0] -= di_[0];
          loc2glob[1] -= di_[0];
        ...
        if (factorize && !Option::get()->set(prefix + "schwarz_method")) {
          ...
          if (a != b && a != C) delete[] a;
          if (DMatrix::n_) Solver<K>::template numfact<S>(std::distance(irn, jcn), irn, jcn, a = b);
      ...
      if (opt.val<char>("krylov_method", HPDDM_KRYLOV_METHOD_GMRES) != HPDDM_KRYLOV_METHOD_NONE) {
        ...
        if (range.size() > 1) {
          ...
          if (it != range.cend()) accumulate -= std::distance(*(it - 1), *it);
        ...
        accumulate = 0;
        for (const int &i : on) g2l.emplace(i - (super::numbering_ == 'F'), accumulate++);
        std::set<int>().swap(on);
        ...
        if (S == 'S') {
          ...
          for (unsigned short i = 0; i < (T == 1 ? off_ : DMatrix::rank_); ++i) {
            ...
            if (table[idx >> 3] & (1 << (idx & 7))) infoRecv.emplace_back(i);
          }
          delete[] table;
          ...
          for (std::pair<const unsigned short, std::vector<int>> &i : send_) std::for_each(i.second.begin(), i.second.end(), [&g2l](int &j) { j = g2l.at(j); });
        accumulate = 0;
        for (std::pair<const int, unsigned short> &i : off) g2l.emplace(i.first - (super::numbering_ == 'F'), accumulate++);
        ...
        *buff_     = new K[accumulate * bs_];
        ...
        off_ = off.size();
        if (DMatrix::rank_ != 0) opt.remove("verbosity");
      } else {
        delete[] di_;
        di_  = nullptr;
        off_ = 0;
      ...
      if (factorize) {
        ...
        if (DMatrix::n_) Solver<K>::template numfact<S>(std::distance(irn, jcn), irn, jcn, b);
        if (b != C) delete[] b;
        Solver<K>::range_ = {(loc2glob[0] - (Solver<K>::numbering_ == 'F')) * bs_, (loc2glob[1] + (Solver<K>::numbering_ == 'C')) * bs_};
  #endif
      }
    ...
    for (unsigned int i = 0; i < nrow; ++i) {
      ...
      for (unsigned int j = 0; j < I[i + 1] - I[i] - di_[i]; ++j) {
        ...
        if (neighbors[k] == DMatrix::rank_) {
          ...
          off[J[k]] = neighbors[k];
          allocation[neighbors[k]] += 1;
          if (S == 'S' && factorize && neighbors[k] > DMatrix::rank_) exchange[neighbors[k]].insert(range_[0] + i);
          if (!r) {
            r = true;
            range.emplace_back(J + k);
          }
    ...
    if (S == 'S') {
      ...
      for (unsigned int i = 0; i < infoRecv.size(); ++i) {
        ...
        std::for_each(send_[infoRecv[index]].begin(), send_[infoRecv[index]].end(), [&g2l, &on, &overlap](int &j) {
          if (factorize && on.find(j) == on.cend()) overlap.insert(j);
          j = g2l.at(j);
        });
      }
      MPI_Waitall(size - infoRecv.size(), rq + infoRecv.size(), MPI_STATUSES_IGNORE);
    ...
    if (factorize && !idx_) {
      ...
      for (const auto &range : {on, overlap})
        for (const int &i : range) *idx_++ = i - (super::numbering_ == 'F');
      for (std::pair<const int, unsigned short> &i : off) *idx_++ = i.first - (super::numbering_ == 'F');
...
public:
  ...
  #endif
        delete[] da_;
      if (communicator_ != DMatrix::communicator_) MPI_Comm_free(&communicator_);
    }
    delete[] rq_;
#else
  ...
  return_type solve(K *rhs, const unsigned short &mu)
  {
    ...
    if (s_) {
      ...
      for (unsigned short i = 0; i < mu; ++i) std::copy_n(out + i * n, dof_ * bs_, rhs + i * n);
      delete[] out;
      delete[] in;
    } else {
      if (mu_ != mu) {
  ...
  decltype(s_) getSubdomain() const { return s_; }
#if !HPDDM_PETSC
  void setParent(decltype(p_) const p) { p_ = p; }
  ...
  typename std::conditional<HPDDM_PETSC, Mat, MatrixCSR<K> *>::type buildMatrix(const InexactCoarseOperator<HPDDM_TYPES_COARSE_OPERATOR(Solver, S, K)> *in, const std::vector<unsigned int> &displs, const unsigned int *const ranges, const std::vector<std::vector<unsigned int>> &off, const std::vector<std::vector<std::pair<unsigned short, unsigned short>>> &reduction = std::vector<std::vector<std::pair<unsigned short, unsigned short>>>(), const std::map<std::pair<unsigned short, unsigned short>, unsigned short> &sizes = std::map<std::pair<unsigned short, unsigned short>, unsigned short>(), const std::unordered_map<unsigned short, std::tuple<unsigned short, unsigned int, std::vector<unsigned short>>> &extra = std::unordered_map<unsigned short, std::tuple<unsigned short, unsigned int, std::vector<unsigned short>>>(), const std::tuple<int *, K *, MPI_Request *> &transfer = std::tuple<int *, K *, MPI_Request *>()) const
  {
    ...
    di[0]             = 0;
    for (unsigned int i = 0; i < in->dof_; ++i) di[i + 1] = di[i] + in->oi_[i + 1] - in->oi_[i] + in->di_[i + 1] - in->di_[i];
    ...
    if (in == this) {
      ...
      if (S == 'S') {
        ...
        for (unsigned int i = 0; i < 2 * in->recv_.size(); ++i) {
          ...
          if (index < in->recv_.size()) delete[] send[index].first;
          else delete[] send[index - in->recv_.size()].second;
        }
        send.clear();
      }
      ...
      for (const std::pair<const unsigned short, std::vector<int>> &p : in->send_) {
        ...
        unsigned int accumulate = 0;
        for (unsigned int i = 0; i < p.second.size(); ++i) accumulate += send.back().first[2 * i] + send.back().first[2 * i + 1];
      ...
      for (unsigned short j = 0; j < in->recv_.size(); ++j) {
        ...
        for (unsigned int j = 0; j < in->recv_[index].second.size(); ++j) {
          ...
          for (unsigned int k = 0; k < recv[0][index].first[2 * j + 1]; ++k) {
            ...
            if (it != nnz.cend() && *it == HPDDM::lround(HPDDM::abs(recv[0][index].second[accumulate + recv[0][index].first[2 * j] * (1 + bss) + k]))) ++onnz;
          }
          for (unsigned int k = in->recv_[index].second[j]; k < in->off_; ++k) di[in->dof_ + displs.back() + k + 1] += recv[0][index].first[2 * j] + onnz;
      ...
      for (unsigned int i = 0; i < 2 * in->send_.size(); ++i) {
        ...
        if (index < in->send_.size()) delete[] send[index].first;
        else delete[] send[index - in->send_.size()].second;
      }
      delete[] rqRecv;
    } else if (!sizes.empty()) {
      ...
      for (unsigned short k = 0; k < rows.size() - 1; ++k) {
        ...
        if (in->bs_ == 1) {
          ...
          accumulate += 1;
          for (unsigned int j = accumulate; j < in->off_ + displs.back(); ++j) di[in->dof_ + j + 1] += row;
        }
      }
    }
    ...
    for (unsigned int i = 0; i < in->dof_; ++i) {
      ...
      if (S == 'S') {
        if (in == this) {
          ...
          while (it != extra.cend()) {
            if (std::get<1>(it->second) <= i && i < (std::get<1>(it->second) + std::get<0>(it->second))) break;
            ++it;
          }
          if (it != extra.cend()) {
    ...
    if (DMatrix::communicator_ != MPI_COMM_NULL) {
      ...
      if (S == 'S') {
        for (const std::pair<const unsigned short, std::vector<int>> &p : send_) {
          ...
          for (unsigned int i = 0; i < displs[j + 1] - displs[j]; ++i)
            for (unsigned int k = 0; k < bs_; ++k) *it++ = (dof_ + displs[j] + i) * bs_ + k;
          for (unsigned int i = 0; i < p.second.size(); ++i)
            for (unsigned int k = 0; k < bs_; ++k) *it++ = p.second[i] * bs_ + k;
          ++j;
        }
      }
      for (const pairNeighbor &p : recv_) {
        ...
        if (S != 'S' && p.first < DMatrix::rank_) {
          for (unsigned int i = 0; i < p.second.size(); ++i)
            for (unsigned int k = 0; k < bs_; ++k) r[j][i * bs_ + k] = (dof_ + p.second[i]) * bs_ + k;
          for (unsigned int i = 0; i < send_[p.first].size(); ++i)
            for (unsigned int k = 0; k < bs_; ++k) r[j][(p.second.size() + i) * bs_ + k] = send_[p.first][i] * bs_ + k;
        } else {
          std::vector<int>::iterator it = r[j].begin();
          if (S != 'S')
            for (unsigned int i = 0; i < send_[p.first].size(); ++i)
              for (unsigned int k = 0; k < bs_; ++k) *it++ = send_[p.first][i] * bs_ + k;
          else {
            for (unsigned int i = 0; i < off[j - send_.size()].size(); ++i) {
              for (unsigned int k = 0; k < bs_; ++k) *it++ = off[j - send_.size()][i] * bs_ + k;
            }
          }
          for (unsigned int i = 0; i < p.second.size(); ++i)
            for (unsigned int k = 0; k < bs_; ++k) *it++ = (dof_ + displs.back() + p.second[i]) * bs_ + k;
        }
        ++j;
      }
#if !HPDDM_PETSC
      ...
      PetscReal *d;
      if (!std::is_same<PetscScalar, PetscReal>::value) d = new PetscReal[m]();
      else {
...
#endif
        {
          ...
          if (level->threshold >= PetscReal()) {
            ...
            while (i < level->nu) {
              ...
              if (HPDDM::abs(eigr) > level->threshold) break;
              ++i;
            }
            level->nu = i;
          }
...
private:
  ...
  void exchange(const K *const in, K *const out, const unsigned short &mu = 1) const
  {
    for (unsigned short nu = 0; nu < mu; ++nu) {
      ...
      if (T == 'N')
        for (const std::pair<unsigned short, std::vector<int>> &p : send_) {
          for (unsigned int j = 0; j < p.second.size(); ++j) std::copy_n(in + (nu * dof_ + p.second[j]) * bs_, bs_, buff_[i] + j * bs_);
        ...
        i = 0;
        while (i < recv_.size()) {
          for (unsigned int j = 0; j < recv_[i].second.size(); ++j) std::copy_n(o_ + (nu * off_ + recv_[i].second[j]) * bs_, bs_, buff_[i] + j * bs_);
          MPI_Isend(buff_[i], recv_[i].second.size() * bs_, Wrapper<K>::mpi_type(), recv_[i].first, 20, communicator_, rq_ + i);
          ++i;
        }
      }
      if (nu != mu - 1) wait<T>(T == 'N' ? o_ + nu * off_ * bs_ : out + nu * dof_ * bs_);
    }
  }
  template <char T>
  void wait(K *const in) const
  {
    if (T == 'N') {
      for (unsigned short i = 0; i < recv_.size(); ++i) {
        ...
        for (unsigned int j = 0; j < recv_[index].second.size(); ++j) std::copy_n(buff_[index] + j * bs_, bs_, in + recv_[index].second[j] * bs_);
      }
