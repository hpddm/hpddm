  ...
  if (data->is || (ismatis && data->N > 1)) {
    ...
    if (data->N > 1 && (data->aux || ismatis || algebraic || ctx)) {
      ...
      if (ctx) {
        ...
        std::swap(uis, data->is);
        swap = PETSC_TRUE;
        if (std::get<0>(*ctx)[1]) PetscCall(PCASMSetType(data->levels[0]->pc, PC_ASM_NONE)); /* "Neumann--Neumann" preconditioning with overlap and a Boolean partition of unity */
        else {
          PetscContainer container;
          ...
          PetscCheck(*threshold >= PetscReal() && *threshold <= static_cast<PetscReal>(1.0), PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "Current value %g not in allowed range [0; 1]", (double)*threshold);
          PetscCall(PetscContainerCreate(PetscObjectComm((PetscObject)A), &container));
        ...
        PetscCall(PCSetOptionsPrefix(inner, ((PetscObject)ksp[0])->prefix));
        PetscCall(PetscObjectTypeCompare((PetscObject)inner, PCSHELL, &flg));
        if (flg) PetscCall(PCSetType(inner, PCNONE)); /* need to reset the inner PCSHELL, do this by setting it to PCNONE so that PCDestroy() is called */
