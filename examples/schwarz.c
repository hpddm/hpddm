...
int main(int argc, char **argv)
{
  ...
  HpddmOptionParse(opt, argc, argv, rankWorld == 0);
  {
    ...
    val[1]  = "nonuniform=(0|1)";
    desc[1] = "Use a different number of eigenpairs to compute on each subdomain.";
  ...
  if (sizeWorld > 1) {
    ...
    if (rankWorld != 0) HpddmOptionRemove(opt, "verbosity");
    ...
    free(storage);
    if (HpddmOptionVal(opt, "geneo_nu") == 0) HpddmDestroyVectors(HpddmSchwarzPreconditioner(A));
    ...
    for (unsigned short nu = 0; nu < mu; ++nu) nrmb[nu] = nrm2(&ndof, f + nu * ndof, &one);
    ...
    for (unsigned short nu = 0; nu < mu; ++nu) {
      ...
      if (mu > 1) printf(" (rhs #%d)", nu + 1);
      printf("\n");
      if (nrmAx[nu] / nrmb[nu] > (sizeof(underlying_type) == sizeof(double) ? 1.0e-6 : 1.0e-2)) status = 1;
    }
    free(tmp);
    free(nrmb);
