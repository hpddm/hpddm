        ...
        if(opt.app()["diagonal_scaling"] == 0)
            ...
            for(int i = 0; i < n_; ++i) {
                ...
                for(unsigned short nu = 0; nu < mu; ++nu)
                    out[nu * n_ + i] = in[nu * n_ + i] / A->a_[mid];
            }
        }
        return 0;
    }
    ...
    do {
        ...
        int mu = opt.app()["mu"];
        {
            ...
            for(unsigned int i = 0; i < nnz; ++i)
                Mat->a_[i] = HPDDM::template sto<double>(s[3 + i]);
            for(unsigned int i = 0; i < nnz; ++i)
                Mat->ja_[i] = HPDDM::template sto<int>(s[nnz + 3 + i]);
            for(unsigned int i = 0; i < n + 1; ++i)
                Mat->ia_[i] = HPDDM::template sto<int>(s[2 * nnz + 3 + i]);
            rhs = new K[mu * n];
        ...
        if(mu > 1)
            for(unsigned short nu = 1; nu < mu; ++nu)
                std::copy_n(rhs, Mat->n_, rhs + nu * Mat->n_);
