...
int main(int argc, char **argv)
{
  ...
  std::string prefix;
  if (opt.prefix("prefix").size()) prefix = opt.prefix("prefix");
  if (rankWorld != 0) {
    opt.remove("verbosity");
    if (prefix.size() > 0) opt.remove(prefix + "verbosity");
  }
  ...
  if (sizeWorld > 1) {
    ...
    if (it > ((HPDDM_MIXED_PRECISION || opt.val<char>(prefix + "krylov_method", HPDDM_KRYLOV_METHOD_GMRES) == HPDDM_KRYLOV_METHOD_BFBCG) ? 60 : 45)) status = 1;
    else {
      for (unsigned short nu = 0; nu < mu; ++nu)
    ...
    for (unsigned short nu = 0; nu < mu; ++nu) {
      ...
      if (mu > 1) std::cout << " (rhs #" << nu + 1 << ")";
      std::cout << std::endl;
      if (nrmAx[nu] / nrmb[nu] > (std::is_same<double, HPDDM::underlying_type<K>>::value ? 1.0e-6 : 1.0e-2)) status = 1;
    }
    if (it > 75) status = 1;
    delete[] tmp;
    delete[] nrmb;
