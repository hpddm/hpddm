...
int main(int argc, char **args)
{
  ...
  PetscCall(MatSetOption(T, MAT_STRUCTURE_ONLY, PETSC_TRUE));
  PetscCall(MatDuplicate(A, MAT_DO_NOT_COPY_VALUES, &perm));
  PetscCall(MatSetOption(perm, MAT_STRUCTURE_ONLY, PETSC_TRUE));
...
/*TEST
   ...
   test:
      ...
      nsize: 4
      args: -pc_type hpddm -pc_hpddm_levels_1_eps_nev 80 -pc_hpddm_block_splitting -pc_hpddm_levels_1_sub_pc_type lu -pc_hpddm_levels_1_eps_gen_non_hermitian -pc_hpddm_levels_1_sub_pc_factor_mat_solver_type mumps -pc_hpddm_levels_1_sub_mat_mumps_icntl_24 1 -pc_hpddm_levels_1_st_share_sub_ksp {{true false}shared output} -pc_hpddm_coarse_mat_type baij -ksp_error_if_not_converged -ksp_pc_side right -ksp_max_it 20 -ksp_orthogonalization mgs -pc_hpddm_levels_1_st_pc_factor_shift_type nonzero
